...
class BackupAndRestoreSection extends HookConsumerWidget {
  ...
  Future<PlatformFile?> getBackupFile(BuildContext context, Toast toast) async {
  ...
  Future<String?> backupFilePicker(WidgetRef ref, BuildContext context) async {
    Toast toast = ref.read(toastProvider(context));
    ...
    final validateResult = await AsyncValue.guard(() =>
        ref.read(settingsRepositoryProvider).validateBackup(backupFile).first);
