...
class ExtensionProvider extends GetConnect {
  ...
  void onInit() {
    httpClient.defaultDecoder = (map) {
      ...
      if (map is List) return map.map((item) => Extension.fromJson(item)).toList();
    };
  ...
  Future installExtension(String pkgName) async {
    final response = await get( _localStorageService.baseURL + extensionURL + '/install/$pkgName');
    return response.body;
  }

  Future updateExtension(String pkgName) async {
    final response = await get( _localStorageService.baseURL + extensionURL + '/update/$pkgName');
    return response.body;
  }
  Future uninstallExtension(String pkgName) async {
    final response = await get( _localStorageService.baseURL + extensionURL + '/uninstall/$pkgName');
    return response.body;
  }
...
Future<Response> installExtensionFile(File file) async {
    ...
    return await post(
      ...
      FormData({
        'file': MultipartFile(bytefile, filename: file.path.split("/").last.split('\\').last)
      }),
    );
  }
}
